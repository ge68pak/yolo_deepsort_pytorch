...
def detect(cfgfile, weightfile, imgfolder):
    ...
    for imgname in imgfiles:
        ...
        #for i in range(2):
        start = time.time()
        boxes = do_detect(m, sized, 0.5, 0.4, use_cuda)   # finish total lo process to get predicted BBox
        finish = time.time()
            #if i == 1:
        print('%s: Predicted in %f seconds.' % (imgfile, (finish-start)))

        class_names = load_class_names(namesfile)    # get line information(class names) from cfg/coco.names
        img = plot_boxes(img, boxes, 'results/{}'.format(os.path.basename(imgfile)), class_names)
        img = np.array(img)
