...
class YoloLayer(nn.Module):
    ...
    def get_mask_boxes(self, output):
        ...
        masked_anchors = []
        for m in self.anchor_mask: #  找到属于该特征mask 对应原图上的anchor shape
            masked_anchors += self.anchors[m * self.anchor_step:(m + 1) * self.anchor_step]
        masked_anchors = [anchor / self.stride for anchor in masked_anchors]  #/stride 对应feature map的anchor shape

        masked_anchors = torch.FloatTensor(masked_anchors).to(self.device)
        num_anchors = torch.IntTensor([len(self.anchor_mask)]).to(self.device)   #3
        return {'x': output, 'a': masked_anchors, 'n': num_anchors}

    def build_targets(self, pred_boxes, target, anchors, nA, nH, nW):
        nB = target.size(0)
        anchor_step = anchors.size(1)  # anchors[nA][anchor_step]  3*2
        ...
        twidth, theight = self.net_width / self.stride, self.net_height / self.stride #找到ground truth长宽 （original image->feature map）
        ...
        for b in range(nB):#对于每一个batch图片
            ...
            for t in range(50):
                if tbox[t][1] == 0:
                    break
                gx, gy = tbox[t][1] * nW, tbox[t][2] * nH  # ound truth Bounding Box gx gy gw gh
                gw, gh = tbox[t][3] * twidth, tbox[t][4] * theight
                cur_gt_boxes = torch.FloatTensor([gx, gy, gw, gh]).repeat(nAnchors, 1).t()
                cur_ious = torch.max(cur_ious, multi_bbox_ious(cur_pred_boxes, cur_gt_boxes, x1y1x2y2=False)) # IOU of predict and gt
            ...
            for t in range(50):
                if tbox[t][1] == 0:
                    break
                nGT += 1
                gx, gy = tbox[t][1] * nW, tbox[t][2] * nH # ground truth Bounding Box gx gy gw gh
                gw, gh = tbox[t][3] * twidth, tbox[t][4] * theight
                ...
                _, best_n = torch.max(multi_bbox_ious(tmp_gt_boxes, anchor_boxes, x1y1x2y2=False), 0) #只取anchor与gt的IOU较大的那一种shape

                gt_box = torch.FloatTensor([gx, gy, gw, gh])     #ground truth bounding box
                ...
                tcoord[0][b][best_n][gj][gi] = gx - gi       #gx=gx-Cx
                tcoord[1][b][best_n][gj][gi] = gy - gj       #gy=gy-Cy
                tcoord[2][b][best_n][gj][gi] = math.log(gw / anchors[best_n][0])  #gw=ln(gw/Pw)
                tcoord[3][b][best_n][gj][gi] = math.log(gh / anchors[best_n][1])  #gh=ln(gh/Ph)
                tcls[b][best_n][gj][gi] = tbox[t][0]          #gt class
                tconf[b][best_n][gj][gi] = iou if self.rescore else 1.  #gt confidence
    ...
    def forward(self, output, target):
        # output : B*Asx(4+1+num_classes)*H*W ch As:erery feature map has 3 achor shape
        mask_tuple = self.get_mask_boxes(output)  #{'x': output, 'a': masked_anchors, 'n': num_anchors}
        t0 = time.time()
        ...
        anchor_step = mask_tuple['a'].size(0) // nA  #6//3
        anchors = mask_tuple['a'].view(nA, anchor_step).to(self.device)  # 3种anchor*每种anchor有2个值 分别为长宽
        ...
        coord = output.index_select(2, ix[0:4]).view(nB * nA, -1, nH * nW).transpose(0, 1).contiguous().view(-1,
                                                                                                             cls_anchor_dim)  # tx, ty, tw, th
        coord[0:2] = coord[0:2].sigmoid()  # tx, ty
        conf = output.index_select(2, ix[4]).view(nB, nA, nH, nW).sigmoid()   # confidence
        cls = output.index_select(2, cls_grid)      # classification
        cls = cls.view(nB * nA, nC, nH * nW).transpose(1, 2).contiguous().view(cls_anchor_dim, nC)

        t1 = time.time()
        grid_x = torch.linspace(0, nW - 1, nW).repeat(nB * nA, nH, 1).view(cls_anchor_dim).to(self.device)   # Cx Cy
        grid_y = torch.linspace(0, nH - 1, nH).repeat(nW, 1).t().repeat(nB * nA, 1, 1).view(cls_anchor_dim).to(
            self.device)
        anchor_w = anchors.index_select(1, ix[0]).repeat(1, nB * nH * nW).view(cls_anchor_dim)   #Pw
        anchor_h = anchors.index_select(1, ix[1]).repeat(1, nB * nH * nW).view(cls_anchor_dim)   #Ph

        pred_boxes[0] = coord[0] + grid_x    #bx=tx+Cx
        pred_boxes[1] = coord[1] + grid_y    #by=ty+Cy
        pred_boxes[2] = coord[2].exp() * anchor_w  #bw=Pw*exp(tw)
        pred_boxes[3] = coord[3].exp() * anchor_h  #bh=Ph*exp(th)
