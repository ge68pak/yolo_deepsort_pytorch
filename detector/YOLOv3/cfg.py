...
def parse_cfg(cfgfile):
    blocks = []   #用来保存cfg件中每一个模块的信息 （[convolutional] [shortcut] [yolo] [route] [upsample]）
    fp = open(cfgfile)
    block = None
    line = fp.readline()
    while line != '':    #排除掉空行
        line = line.rstrip()
        if line == '' or line[0] == '#':   #排除掉注释行
            line = fp.readline()
            continue
        elif line[0] == '[':   #找到每一个模块的开头
            if block:
                blocks.append(block)
            block = dict()     #每一个模块对应一个字典 新建该模块的空字典
            ...
            if key == 'type':
                key = '_type'
            value = value.strip()
            block[key] = value   #ey:value
        line = fp.readline() #继续读取下一行

    if block:
        blocks.append(block)
    fp.close()
    return blocks    #[{key:value,key:value.....},{key:value,key:value.....},{key:value,key:value.....}.......]
