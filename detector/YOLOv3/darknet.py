...
class Darknet(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs = dict()   #记录所有的模块的输出 方便route和residual直接使用
        out_boxes = dict()
        outno = 0
    ...
    def create_network(self, blocks):  #blocks=[{key:value,key:value.....},{key:value,key:value.....},{key:value,key:value.....}.......]
        models = nn.ModuleList()       #逐model逐model的搭网络 最后的总网络是models
    
        prev_filters = 3  #pre在不断更新
        out_filters =[]   #out用于存储所有记录
        prev_stride = 1
        out_strides = []
        conv_id = 0
        ind = -2
        for block in blocks:
            ind += 1
            if block['type'] == 'net':   #[convolutional] [shortcut] [yolo] [route] [upsample]
                prev_filters = int(block['channels'])   #3
                self.width = int(block['width'])        #416
                self.height = int(block['height'])      #416
                continue
                ...
                activation = block['activation']
                model = nn.Sequential()   #一个convenlutional模块：conv2d+BN+leaky ReLu
                if batch_normalize:
                ...
                layers = block['layers'].split(',')   #layers = -4 layers = -1, 61 layers = -4 layers = -1, 36
                ind = len(models) #截至到现在所有模块的数量
                layers = [int(i) if int(i) > 0 else int(i)+ind for i in layers]
                if len(layers) == 1:
                ...
                anchors = block['anchors'].split(',')  #anchors = 10,13,  16,30,  33,23,  30,61,  62,45,  59,119,  116,90,  156,198,  373,326
                anchor_mask = block['mask'].split(',')   #mask = 0,1,2
                yolo_layer.anchor_mask = [int(i) for i in anchor_mask]
                yolo_layer.anchors = [float(i) for i in anchors]
                yolo_layer.num_classes = int(block['classes'])  #classes=80
                yolo_layer.num_anchors = int(block['num'])   #num=9
                yolo_layer.anchor_step = len(yolo_layer.anchors)//yolo_layer.num_anchors
                try:
                    yolo_layer.rescore = int(block['rescore'])
                except:
                    pass
                yolo_layer.ignore_thresh = float(block['ignore_thresh'])   #ignore_thresh = .5
                yolo_layer.truth_thresh = float(block['truth_thresh'])    #truth_thresh = 1
